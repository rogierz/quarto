...
class PlayerParams(list):
    ...
    def __invert__(self):
        ...
        if random.random() <= MUTATION_RATE:
            ...
            for i in range(len(new_self)):
                ...
                new_self[i] = new_self[i] + np.random.normal(new_self.mu[i], new_self.sigma[i])
    ...
    def __xor__(self, other):
        ...
        new_born = PlayerParams([np.clip(self[i]*w[0] + other[i]*w[1], 0, 1).ravel() for i in range(len(self))])
        return new_born
...
class EvolutionaryPlayer(BasePlayer):
    ...
    def cook_status(self):
        ...
        for piece in available_pieces:
           piece_traits = list(map(lambda i: int(piece & (1 << i) == (1 << i)), [0, 1, 2, 3]))
        ...
        diags_counter = np.array([indexes_not_empty.trace(), np.fliplr(indexes_not_empty).trace()])
        ...
        diag_stats[1] = self._reduce_array(np.fliplr(self._game._board).diagonal())
    ...
    def _reduce_array(self, array):
        ...
        for elem in array:
            if elem == -1:
                continue
            piece_traits = list(map(lambda i: int(elem & (1 << i) == (1 << i)), [0, 1, 2, 3]))
    ...
    def choose_piece(self) -> int:
        ...
        max_length = max(length_status[0].max(), length_status[1].max(), length_status[2].max())
        for i, dim in enumerate(length_status):
            if max_length in dim:
                ...
                if i == 0:
                    ...
                    extracted_dim = self._game._board.diagonal() if dim_index == 0 else np.fliplr(self._game._board).diagonal()
                break
        ...
        if (extracted_dim != -1).any():
            ...
            good_choices = list(filter(lambda x: (~((x & bit_mask) ^ bit_filter) & bit_mask) == bit_mask, status['available_pieces']))
    ...
    def place_piece(self) -> tuple[int, int]:
        ...
        if in_mood and random.random() <= self._params.aggressive:
            obj_length = max(length_status[0].max(), length_status[1].max(), length_status[2].max())
        elif not in_mood and random.random() <= self._params.defensive:
            obj_length = min(length_status[0].min(), length_status[1].min(), length_status[2].min())
        else:
            return position
