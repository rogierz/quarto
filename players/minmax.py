...
class MinmaxPlayer(BasePlayer):
    ...
    def _minmax(self):
        if len(self._game.available_pieces) < self.max_depth:
            (position, piece), _ = self.__minmax_r(
                self.get_game(), player=self.moving_index)
            self._chosen_position = position
            self._chosen_piece = piece
        else:
            self._chosen_position = random.choice(
                self._game.available_position)
    ...
    def __minmax_r(
        ...
        val = (
            (self._game.available_position[0], None), -1) if maximising_player else (
            (self._game.available_position[0], None), 1)
        for piece, position in product(
                state.available_pieces, state.available_position):
            ...
            if player == self.moving_index:
                val = max(((position, piece), ns_value),
                          val, key=lambda x: x[1])
                alpha = max(alpha, ns_value)
            else:
                val = min(((position, piece), ns_value),
                          val, key=lambda x: x[1])
